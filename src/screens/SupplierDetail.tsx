...
import { useSupplierBalance } from '../hooks/useSuppliers';
...
export function SupplierDetail() {
  ...
  const deliveries = (id ? state.supplierDeliveries[id] : []) ?? [];
  const payments = (id ? state.supplierPayments[id] : []) ?? [];
  ...
  const handleDeleteDelivery = async (entryId: string) => {
    try {
      await removeDeliveryEntry(entryId, supplier.id, 'supplier');
      showToast(t('deletedSuccessfully'));
    } catch {
      showToast(t('errorOccurred'), 'error');
    }
  ...
  const handleDeletePayment = async (entryId: string) => {
    try {
      await removePaymentEntry(entryId, supplier.id, 'supplier');
      showToast(t('deletedSuccessfully'));
    } catch {
      showToast(t('errorOccurred'), 'error');
    }
      ...
      <BottomSheet isOpen={showDeliverySheet} onClose={() => setShowDeliverySheet(false)} title={t('deliveryEntry')}>
        <SupplierDeliveryForm
          ...
          onSave={async (litres, rate, date, note) => {
            await createDeliveryEntry({ type: 'supplier', partyId: supplier.id, date, litres, rate, note });
      ...
      <BottomSheet isOpen={showPaymentSheet} onClose={() => setShowPaymentSheet(false)} title={t('paymentEntry')}>
        <SupplierPaymentForm
          ...
          onSave={async (amount, date, note) => {
            await createPaymentEntry({ type: 'supplier', partyId: supplier.id, date, amount, note });
            showToast(t('savedSuccessfully'));
...
function EditSupplierForm({
  ...
  supplier: import('../db/index').Supplier;
  ...
  const [rate, setRate] = useState(String(supplier.defaultRate));
  const [saving, setSaving] = useState(false);
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          onClick={async () => {
            if (!name.trim()) return;
            setSaving(true);
            await onSave({ name: name.trim(), phone: phone.trim() || undefined, defaultRate: parseFloat(rate) || supplier.defaultRate });
            setSaving(false);
          }}
